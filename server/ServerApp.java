...
import services.LogsToWordService;
...
public class ServerApp {
    ...
    private class ClientHandler implements Runnable {
        ...
        private boolean login(BufferedReader in, PrintWriter out) throws IOException {
            ...
            out.println("Login successful! Hello, " + emp.getFullName() + " (Role: " + emp.getRole() + ")");
            return true;
        }
        ...
        private String logsToWordCommand() {
            ...
            try {
                LogsToWordService.convertToDoc(branchFile.getAbsolutePath(), productFile.getAbsolutePath(), outputDoc);
            } catch (IOException e) {
                e.printStackTrace();
        ...
        private String addEmployeeCommand(String[] parts) {
            ...
            Role newEmpRole;
            try {
                newEmpRole = Role.valueOf(roleStr);
            } catch (Exception e) {
            ...
            Employee newEmp = new Employee(fullName, id, phone, bankAcc, branch, empNum, newEmpRole);
            ...
            String logMsg = String.format("ADMIN '%s' hired employee '%s' (ID=%s) in branch '%s'",
                    loggedInEmployee.getFullName(), newEmp.getFullName(), newEmp.getId(), newEmp.getBranch());
            logAction(logMsg);
        ...
        private String sellCommand(String[] parts) {
            ...
            BaseCustomer customer = customerService.getCustomerById(custId);
            if (customer == null) {
            ...
            // Log the SELL
            String logMsg = String.format("SELL: Employee '%s' in branch '%s' sold productId='%s' (qty=%d) to customer '%s'",
                    loggedInEmployee.getFullName(), loggedInEmployee.getBranch(),
                    productId, qty, customer.getFullName());
            logAction(logMsg);

            return "SUCCESS: Sold " + qty + " of " + productId + " to " + custId
                   + " (" + customer.getFullName() + "). Final price: " + finalPrice;
        }
        ...
        private String buyProductCommand(String[] parts) {
            ...
            String empBranch = loggedInEmployee.getBranch();
            ...
            if (existingProduct != null) {
                ...
                // Log the BUY
                String logMsg = String.format("BUY: Employee '%s' in branch '%s' purchased more of productId='%s' (qty=%d)",
                ...
                String logMsg = String.format("BUY: Employee '%s' in branch '%s' created new product '%s' (id=%s) with qty=%d",
        ...
        private String addCustomerCommand(String[] parts) {
            ...
            String logMsg = String.format("Employee '%s' (Role=%s) added new customer '%s' (ID=%s)",
                    loggedInEmployee.getFullName(), loggedInEmployee.getRole(), cust.getFullName(), cust.getId());
            logAction(logMsg);
        ...
        private String showEmployees() {
            ...
            for (Employee emp : all) {
                sb.append(String.format("%-20s %-10s %-15s %-15s %-10s %-10d %-10s\n",
                        emp.getFullName(), emp.getId(), emp.getPhone(), emp.getBankAccount(),
                        emp.getBranch(), emp.getEmployeeNumber(), emp.getRole()));
            }
            return sb.toString();
        }
        ...
        private String showProducts() {
            if (loggedInEmployee.getRole() == Role.ADMIN) {
                ...
                String branch = loggedInEmployee.getBranch();
        ...
        private String showCustomers() {
            ...
            List<BaseCustomer> allCustomers = customerService.listAllCustomers();
            ...
            sb.append(String.format("%-20s %-10s %-15s %-15s\n",
                    "Name", "ID", "Phone", "Type"));
            sb.append("-------------------------------------------------------------\n");
            for (BaseCustomer cust : allCustomers) {
                sb.append(String.format("%-20s %-10s %-15s %-15s\n",
                        cust.getFullName(), cust.getId(), cust.getPhone(), cust.getType()));
            }
            return sb.toString();
        }
        ...
        private String startChatCommand(String[] parts) {
            ...
            String myBranch = loggedInEmployee.getBranch();
            ...
            String logMsg = String.format("CHAT: Employee '%s' (branch='%s') STARTED a chat with branch='%s'; chatId='%s'",
        ...
        private String joinChatCommand(String[] parts) {
            ...
            String userBranch = loggedInEmployee.getBranch();
            ...
            return "You joined chat " + chatId + ". Current chat set. Branches in chat: " + session.getBranchesInvolved();
        }
    ...
    private void loadBranchesFromFile(String filePath) {
        ...
        for (String objStr : objects) {
            ...
            if (!branchJson.startsWith("{")) branchJson = "{" + branchJson;
            if (!branchJson.endsWith("}"))   branchJson += "}";
    ...
    private Employee parseEmployeeFromJson(String json) {
        ...
        Employee e = new Employee(fullName, id, phone, bankAccount, branch, empNum, role);
    ...
    private void loadProductsFromFile(String filePath) {
        ...
        for (String objStr : objectStrings) {
            ...
            if (!prodJson.startsWith("{")) prodJson = "{" + prodJson;
            if (!prodJson.endsWith("}"))   prodJson += "}";
    ...
    private void loadCustomersFromFile(String filePath) {
        ...
        for (String objStr : objectStrings) {
            ...
            if (!custJson.startsWith("{")) custJson = "{" + custJson;
            if (!custJson.endsWith("}"))   custJson += "}";
            BaseCustomer c = parseCustomerFromJson(custJson);
            if (c != null) {
    ...
    private BaseCustomer parseCustomerFromJson(String json) {
        String name  = extractJsonStringValue(json, "name");
        String id    = extractJsonStringValue(json, "id");
        String phone = extractJsonStringValue(json, "phone");
    ...
    private String buildEmployeesJson(List<Employee> employees) {
        ...
        for (int i = 0; i < employees.size(); i++) {
            ...
            sb.append("    \"phone\": \"").append(e.getPhone()).append("\",\n");
            sb.append("    \"bankAccount\": \"").append(e.getBankAccount()).append("\",\n");
            sb.append("    \"branch\": \"").append(e.getBranch()).append("\",\n");
    ...
    private String buildCustomersJson(List<BaseCustomer> customers) {
        StringBuilder sb = new StringBuilder();
        sb.append("[\n");
        for (int i = 0; i < customers.size(); i++) {
            BaseCustomer c = customers.get(i);
            sb.append("  {\n");
